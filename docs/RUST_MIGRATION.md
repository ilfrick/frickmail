...
Published `caac4e08b` to `master` + `rust-full-migration` on `origin` and
...
Published `012ce2ff6` to `master` + `rust-full-migration` on `origin` and
`gitea` (all 4 tips identical, verified via `ls-remote`).
...
Published `46bbb03b6` to `master` + `rust-full-migration` on `origin` and
`gitea` (all 4 tips identical, verified via `ls-remote`).
...
`importSmimeP12`, `deleteSmimeCert`) against the v1 S/MIME API. All
rendering passes through the shared `escapeHtml`; the collectors take
stub-able roots. Sign/verify stay out of this screen (message-content
ops, like compose attachments).

Verification so far (host node, Docker-free like prior UI slices):
`node --test frickmail-ui/v1/js/smime.test.mjs` 12 passed / 0 failed;
full `node --test frickmail-ui/v1/js/*.test.mjs` 131 passed / 0 failed;
`npx eslint` on touched JS clean; `build.mjs` ships `smime.js`
automatically (tests excluded, nav wired in the bundle); no legacy
product names in the new files.
...
`POST /api/frickmail/v1/admin/logout` clears only that flag; anonymous and
authenticated `GET /session` report `is_admin`; and `v1_require_admin` guards
future endpoints with 403s. Wrong/empty/oversize/corrupt-hash tokens are
...
Published `cfd7730fd` to `master` + `rust-full-migration` on `origin` and
`gitea` (all 4 tips identical, verified via `ls-remote`); exact-SHA `rust-ci`
+ `naming` runs terminal success.
...
Implementation commit `0bee0f7315fa0ae54b51caf2845761ef7cce8490` was
published to `master` and `rust-full-migration` on both remotes; live
`git ls-remote` checks confirmed all four tips resolve to that SHA. As a
UI/docs-only change it matches no `rust-ci` path filter; the `naming` gate
passed on both branches
([`34701360369`](https://github.com/ilfrick/frickmail/actions/runs/34701360369),
[`34701362967`](https://github.com/ilfrick/frickmail/actions/runs/34701362967)).
This closing documentation-only amendment intentionally matches no CI path
filter and is expected to produce no GitHub Actions run. The major
remaining gates toward the final Rust-only goal are unchanged (rules UI and
other screens/endpoints, theming removal, cutover validation).
...
Implementation commit `98d62e8812820772d0facc0a5601af0e64699c10` was
published to `master` and `rust-full-migration` on both remotes; live
...
[`34700098358`](https://github.com/ilfrick/frickmail/actions/runs/34700098358)
and `rust-full-migration` run
[`34700101351`](https://github.com/ilfrick/frickmail/actions/runs/34700101351),
and the `naming` gate passed on both branches
([`34700098372`](https://github.com/ilfrick/frickmail/actions/runs/34700098372),
[`34700101374`](https://github.com/ilfrick/frickmail/actions/runs/34700101374));
...
Implementation commit `ea3cad46e0cb6d87244186fb28d0f867f9bd0bad` was
published to `master` and `rust-full-migration` on both remotes; live
`git ls-remote` checks confirmed all four tips resolve to that SHA. As a
UI/docs-only change it matches no `rust-ci` path filter; the `naming` gate
passed on both branches
([`34693851960`](https://github.com/ilfrick/frickmail/actions/runs/34693851960),
[`34693854715`](https://github.com/ilfrick/frickmail/actions/runs/34693854715)).
This closing documentation-only amendment intentionally matches no CI path
filter and is expected to produce no GitHub Actions run. The major
remaining gates toward the final Rust-only goal are unchanged (more v1
screens/endpoints, theming removal, cutover validation).
...
Implementation commit `99c5237e22909d4524ba9aba837ac90b0a2ba489` was
published to `master` and `rust-full-migration` on both remotes; live
`git ls-remote` checks confirmed all four tips resolve to that SHA. As a
UI/docs-only change it matches no `rust-ci` path filter; the `naming` gate
passed on both branches
([`34690863624`](https://github.com/ilfrick/frickmail/actions/runs/34690863624),
[`34690865193`](https://github.com/ilfrick/frickmail/actions/runs/34690865193)).
This closing documentation-only amendment intentionally matches no CI path
filter and is expected to produce no GitHub Actions run. The major
remaining gates toward the final Rust-only goal are unchanged (more v1
screens/endpoints, theming removal, cutover validation).
...
Implementation commit `5d164528e39d73ff88b7d381b257d68121a0674e` was
published to `master` and `rust-full-migration` on both remotes; live
`git ls-remote` checks confirmed all four tips resolve to that SHA. As a
UI/docs-only change it matches no `rust-ci` path filter; the `naming` gate
passed on both branches
([`34690665987`](https://github.com/ilfrick/frickmail/actions/runs/34690665987),
[`34690667374`](https://github.com/ilfrick/frickmail/actions/runs/34690667374)).
This closing documentation-only amendment intentionally matches no CI path
filter and is expected to produce no GitHub Actions run. The major
remaining gates toward the final Rust-only goal are unchanged (more v1
screens/endpoints, theming removal, cutover validation).
...
Implementation commit `c32da715b05cb4b5aa4b0af5bd7970fd9861e777` was
published to `master` and `rust-full-migration` on both remotes; live
`git ls-remote` checks confirmed all four tips resolve to that SHA. As a
UI/docs-only change it matches no `rust-ci` path filter; the `naming` gate
passed on both branches
([`34690174892`](https://github.com/ilfrick/frickmail/actions/runs/34690174892),
[`34690176254`](https://github.com/ilfrick/frickmail/actions/runs/34690176254)).
This closing documentation-only amendment intentionally matches no CI path
filter and is expected to produce no GitHub Actions run. The major
remaining gates toward the final Rust-only goal are unchanged (more v1
...
Implementation commit `edfb110fb643bda7f29fe84997c8a544cdf56748` was
published to `master` and `rust-full-migration` on both remotes; live
`git ls-remote` checks confirmed all four tips resolve to that SHA.
Exact-SHA GitHub `rust-ci` passed for that SHA on `master` run
[`34664458405`](https://github.com/ilfrick/frickmail/actions/runs/34664458405)
and `rust-full-migration` run
[`34664461174`](https://github.com/ilfrick/frickmail/actions/runs/34664461174),
and the `naming` gate passed on both branches
([`34664458466`](https://github.com/ilfrick/frickmail/actions/runs/34664458466),
[`34664461175`](https://github.com/ilfrick/frickmail/actions/runs/34664461175));
all runs reported only the known nonblocking Node.js 20 deprecation
annotation. This closing documentation-only amendment intentionally matches no
CI path filter and is expected to produce no GitHub Actions run. The major
remaining gates toward the final Rust-only goal are unchanged (more v1
...
Implementation commit `b08a1b9cb9e7e1d0ed0c3fe24044bded5f9cfee9` was
published to `master` and `rust-full-migration` on both remotes; live
`git ls-remote` checks confirmed all four tips resolve to that SHA.
Exact-SHA GitHub `rust-ci` passed for that SHA on `master` run
[`34661866842`](https://github.com/ilfrick/frickmail/actions/runs/34661866842)
and `rust-full-migration` run
[`34661870932`](https://github.com/ilfrick/frickmail/actions/runs/34661870932),
and the `naming` gate passed on both branches
([`34661866854`](https://github.com/ilfrick/frickmail/actions/runs/34661866854),
[`34661870887`](https://github.com/ilfrick/frickmail/actions/runs/34661870887));
all runs reported only the known nonblocking Node.js 20 deprecation
annotation. This closing documentation-only amendment intentionally matches no
CI path filter and is expected to produce no GitHub Actions run. The major
remaining gates toward the final Rust-only goal are unchanged (more v1
...
Implementation commit `c7a79a69cb8fba37deeb8e045d523a1b8416e4ed` was
published to `master` and `rust-full-migration` on both remotes; live
`git ls-remote` checks confirmed all four tips resolve to that SHA.
Exact-SHA GitHub `rust-ci` passed for that SHA on `master` run
[`34619158241`](https://github.com/ilfrick/frickmail/actions/runs/34619158241)
and `rust-full-migration` run
[`34619167650`](https://github.com/ilfrick/frickmail/actions/runs/34619167650),
and the `naming` gate passed on both branches
([`34619158311`](https://github.com/ilfrick/frickmail/actions/runs/34619158311),
[`34619167639`](https://github.com/ilfrick/frickmail/actions/runs/34619167639));
all runs reported only the known nonblocking Node.js 20 deprecation
annotation. This closing documentation-only amendment intentionally matches no
CI path filter and is expected to produce no GitHub Actions run. The major
remaining gates toward the final Rust-only goal are unchanged (more v1
