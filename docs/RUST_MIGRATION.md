...
2. Complete native parity for legacy `MessageList`, `Message`, and folder status
   ...
   7377 folder scopes, threading, date grouping, previews, and server-side
   UID/cache behavior match the PHP runtime; native search, hidden-deleted
   filtering, date criteria, application/domain-specific search settings, and
   capability-gated sorting are staged.
