...
1. Complete the server-side GnuPG keyring/signing/encryption flow and direct client-supplied
   S/MIME certificate/private-key signing. Client-supplied OpenPGP
   `signed`/`encrypted` MIME content, selected-account S/MIME signing/encryption, OAuth SMTP,
