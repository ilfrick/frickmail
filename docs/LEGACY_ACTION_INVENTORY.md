...
| `MessageList` | `dev/Stores/User/Messagelist.js` | compat-known | Native implementation is staged but dispatch remains disabled until sort/search/threading, complete hidden-deleted filtering, date grouping, RawKey GET cache paths, previews, and full cache parity match legacy PHP behavior. `dateTimestamp`/`dateTimestampSource` parsing, including the INTERNALDATE fallback, BODYSTRUCTURE-based attachment presence and metadata, MailSo-compatible encrypted boolean metadata, MailSo-compatible spam metadata, MailSo-compatible limit normalization, search trimming, sort and limited metadata normalization, subject trimming, flag alias normalization, `References` whitespace normalization, raw Bcc/Sender/Delivered-To/read-receipt metadata, fetched-window `\Deleted` suppression, exact-`INBOX` new-message probing, thread-view `newMessages` suppression, the internal legacy collection/message JSON adapter, and the selected-account POST request/fetcher path are staged internally; live dispatch remains pending. |
