...
| `MessageList` | `dev/Stores/User/Messagelist.js` | compat-known | Native implementation is staged but dispatch remains disabled until sort/search/threading, hidden-deleted filtering, date grouping, RawKey GET cache paths, previews, and full cache parity match legacy PHP behavior. `dateTimestamp`/`dateTimestampSource` parsing, including the INTERNALDATE fallback, BODYSTRUCTURE-based attachment presence, MailSo-compatible limit normalization, search trimming, sort and limited metadata normalization, and thread-view `newMessages` suppression are staged internally; full attachment metadata and legacy JSON shape remain pending. |
