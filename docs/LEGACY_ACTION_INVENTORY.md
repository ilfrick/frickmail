...
| `Message` | `dev/Remote/User/Fetch.js` | partial-native | Native for selected-account POST requests with `folder` and `uid`; reuses Rust IMAP body preview parsing and returns legacy `Object/Message` shape. The PHP-compatible array RawKey GET decoder (`folder`, `uid`, thread flag, account hash), PHP-compatible omission of empty `references`/body/thread fields, removal of the Rust-only `date` field, PHP timestamp source labeling with raw-message `Date` parsing and an `internal` zero fallback when no INTERNALDATE is available, nullable absent EmailId `id`, nullable absent/empty `preview`, nullable unavailable non-raw email/header collections, raw-message fallback email collection, identity-header, read-receipt, attachment metadata population, raw-message header and parameter collection population, decoded raw scalar/address/text-list header values, top-level raw-message `multipart/encrypted` flagging, raw-message `draftInfo`, MailSo-compatible 100-entry email collection caps, normalized IMAP flag propagation, and PHP-compatible Message HTTP cache validator/header emission are staged. PGP/S/MIME bodystructure crypto metadata, non-raw IMAP header/envelope email/header/attachment collections, and exact PHP message model parity remain compatibility work. |
