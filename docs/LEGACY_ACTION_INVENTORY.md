...
| `Message` | `dev/Remote/User/Fetch.js` | partial-native | Native for selected-account POST requests with `folder` and `uid`; reuses Rust IMAP body preview parsing and returns legacy `Object/Message` shape. The PHP-compatible array RawKey GET decoder (`folder`, `uid`, thread flag, account hash) is staged. Full attachment, crypto, header collection, RawKey cache, and exact PHP message model parity remain compatibility work. |
