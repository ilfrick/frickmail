...
| `AttachmentsActions`, `MessageUploadAttachments` | `dev/Common/UtilsUser.js`, `dev/View/Popup/Compose.js` | compat-known | Native prototypes are deliberately not dispatched: the ZIP Raw-download consumer and account-scoped, permission-hardened staging store must be completed before these actions are safe and usable without PHP. |
| `SendMessage` | `dev/View/Popup/Compose.js` | partial-native | Native compose and asynchronous SMTP delivery for password-backed selected accounts. It preserves From/To/Cc/Bcc/Reply-To display names, validates bare envelope addresses, de-duplicates recipients by envelope address, and supports `subject`, `text/plain`, `text/html`, `multipart/alternative`, threading, priority, read-receipt, JSON-LD `linkedData`, legacy `TLS-Required: No`, Autocrypt Level 1 headers, DSN, and SMTP REQUIRETLS. DSN and REQUIRETLS are capability-gated after final TLS negotiation exactly like MailSo: requested DSN adds `RET=HDRS` and per-recipient `NOTIFY=SUCCESS,FAILURE`, requested REQUIRETLS adds its `MAIL FROM` parameter, and unsupported extensions are silently omitted. The cancellable async extension transaction retains hostname-validating TLS over the pinned public address and returns immediately after confirmed DATA acceptance so QUIT cannot create a false unknown outcome. Autocrypt accepts exactly one sender-bound address and exact five-packet transferable public key, verifies its self-signatures, binding type, signing/encryption algorithms, and key flags with rPGP, folds canonical Base64 safely, and enforces the specification's exact 10 KiB emitted-header limit. Full verification runs after authentication in a deadline-bounded two-slot blocking-work pool. For nonempty HTML, active content is parser-sanitized once into a canonical, output-bounded document reused by transport/Sent copies, and JSON-LD is inserted into its head with PHP-compatible solidus escaping; plain-only messages ignore `linkedData` like PHP. Sanitizing runs on Tokio's blocking pool behind a dedicated two-slot admission semaphore; admission and caller wait are bounded to 10 seconds, while timed-out blocking tasks are non-cancellable and retain their permit until exit. A 4 MiB raw-HTML cap and 10,000-tag-marker complexity cap further bound work. A bounded streaming counter charges the exact escaped JSON size against the compose body limit without allocating another pre-authentication buffer. The HTML policy deliberately strips interactive forms, event/data attributes, and unsafe or non-email elements more strictly than PHP; MailSo's `data-x-style-url` and data-image-to-inline-attachment transformations remain part of the unmigrated attachment pipeline. SMTP DNS resolution is deadline-bounded, rejects any private/reserved result, and pins one validated public address while retaining the original hostname for TLS verification; send and draft operations share a bounded semaphore. It deliberately does not retry a whole SMTP transaction on another address after an ambiguous failure, which could duplicate delivery. Transport and Sent copies share one Message-ID and Date; Bcc is omitted in transport but retained in the stored copy. A requested `saveFolder` APPEND must succeed (or return `CantSaveMessage`); fallback occurs only after a definitive rejection, never after a timeout/unknown commit outcome. Reply/forward flagging and draft cleanup remain best-effort. Nonempty unsupported attachment/signing/encryption fields are rejected explicitly; OAuth SMTP and attachment/crypto MIME assembly remain unmigrated. The current PHP basic compose path uses the explicit `from` and `replyTo` payload fields; `identityID` only remains relevant to the unsupported S/MIME certificate-selection path. Compose raw and normalized body/header/recipient bounds prevent the generic 64 MiB request limit from multiplying into excessive memory use. |
| `SaveMessage` | `dev/View/Popup/Compose.js` | partial-native | Natively saves a selected-account draft with `\Seen`, a stable generated `Message-ID`, UID lookup fallback, PHP-compatible `{folder, uid}`/`true` response shapes, and delete-after-append cleanup of the prior draft. It shares the migrated basic compose, JSON-LD, and strictly validated Autocrypt header behavior and rejects unsupported attachment/signing/encryption payloads explicitly so draft data is never silently discarded. Full attachment and crypto MIME parity remains coupled to `SendMessage`. |
...
1. Complete `SendMessage` parity: attachment assembly from cached `tempName`
   uploads, PGP/S-MIME `signed`/`encrypted` payloads and
   S/MIME certificate selection by `identityID`.
2. Complete `SaveMessage` attachment and crypto MIME parity alongside
   `SendMessage`.
