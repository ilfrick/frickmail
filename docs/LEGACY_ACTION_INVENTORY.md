...
| `SendReadReceiptMessage` | `app/libraries/RainLoop/Actions/Messages.php` | partial-native | Native selected-account receipt delivery preflights IMAP before SMTP, builds bounded MailSo-compatible text/plain quoted-printable MIME with a generated Message-ID, applies the configured demo-account recipient policy, then records `$MDNSent`. If the post-delivery IMAP update fails, a bounded account-scoped durable 48-hour suppression marker prevents repeat prompts in native `Message`/`MessageList` responses. The native route currently does **not** expose arbitrary legacy PHP receipt/build/SMTP plugin filters; native generic plugin-hook support remains a migration boundary. |
