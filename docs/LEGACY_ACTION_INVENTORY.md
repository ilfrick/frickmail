...
| `AttachmentsActions` | `dev/Common/UtilsUser.js` | native | Supports the legacy `target=zip` action and its exact `/?/Raw/&q[]=/0/Download/&q[]=/…` follow-up route. It uses selected-account IMAP access, bounded raw-key/item/aggregate input, decoded MIME-part fetching, serialized archive construction, private account-scoped opaque capabilities, atomic ZIP finalization, account ownership checks at download, private no-symlink temporary directories, archive/user/global quotas, expiry cleanup, sanitized ZIP entry/download names, and isolated bounded export tmpfs. Non-ZIP plugin targets remain compatibility hooks. |
...
| `SendReadReceiptMessage` | `app/libraries/RainLoop/Actions/Messages.php` | native | Native selected-account receipt delivery preflights IMAP before SMTP, builds bounded MailSo-compatible text/plain quoted-printable MIME with a generated Message-ID, applies the configured demo-account recipient policy, then records `$MDNSent`. If the post-delivery IMAP update fails, a bounded account-scoped durable 48-hour suppression marker prevents repeat prompts in native `Message`/`MessageList` responses. The `filter.send-read-receipt-message` point is native through the demo-account recipient policy; the `filter.smtp-from`/`filter.message-rcpt` points are native through the opt-in from-address account SMTP switch shared with `SendMessage`. Third-party PHP receipt filters cannot run without the legacy PHP runtime and are retired by architecture. |
...
| `SendMessage` | `dev/View/Popup/Compose.js` | native | Native bounded compose and asynchronous SMTP delivery for password-backed selected accounts. Migrated behavior includes display-name-preserving From/To/Cc/Bcc/Reply-To with bare de-duplicated envelopes; plain/HTML alternatives and MailSo-compatible fallback; threading, priority, read receipts, JSON-LD, `TLS-Required: No`, verified sender-bound Autocrypt, capability-gated DSN/REQUIRETLS, stable transport/Sent identity headers, and definitive-versus-unknown delivery handling. HTML is parser-sanitized in a deadline-bounded blocking pool. Canonical `data:image/<alphanumeric>;base64,...` sources become deduplicated CID-linked MIME parts; effective WHATWG data-scheme detection removes obfuscated, non-image, and noncanonical data URLs. Valid padded/unpadded Base64 with ASCII whitespace is accepted under shared attachment count, byte, and memory admission. Staged regular, inline, CID-linked, and raw-message attachments use MailSo-compatible related/mixed nesting, including as siblings of client-provided OpenPGP signed/encrypted roots; successful delivery deletes exact staged capabilities while SMTP failures and saved drafts retain them. Client OpenPGP and S/MIME signing/encryption preserve outer delivery headers and share one transformed MIME root for SMTP/Sent. Server-side GnuPG detached signing uses an account-scoped private keyring with loopback passphrases delivered via a 0600 passphrase file (never argv/environment), PHP-truthy `signFingerprint` gating (`""`/`"0"` skip, empty passphrase allowed for unprotected keys), strictly validated fingerprints (`0x`/space/colon-tolerant, 8–64 hex, fail-closed), and applies the signed root to both transport and Sent copies. GnuPG encryption supports bounded recipient fingerprints (malformed entries fail instead of subset-encrypting) and the RFC 3156 wrapper embeds GnuPG's armor verbatim (single `BEGIN/END` framing, no doubled header). The active demo-account plugin's external recipient policy is native through configuration. The message filter-hook pipeline is native at the PHP fire points: `filter.build-message` stamps `X-Originating-IP` when `mail.add_x_originating_ip` is set (`add-x-originating-ip-header` port; proxy headers never consulted), `filter.send-message` enforces the demo-account policy, and `filter.smtp-from`/`filter.message-rcpt` switch delivery to the matching account's SMTP endpoint and credentials when `mail.from_address_account_smtp` is enabled with wildcard `mail.from_address_patterns` (`smtp-use-from-adr-account` port, fail-closed on unknown senders by default). Third-party PHP message filters cannot run without the legacy PHP runtime and are retired by architecture. |
| `SaveMessage` | `dev/View/Popup/Compose.js` | native | Natively saves a selected-account draft with `\Seen`, a stable generated `Message-ID`, UID lookup fallback, PHP-compatible `{folder, uid}`/`true` response shapes, and delete-after-append cleanup of the prior draft. It shares the migrated basic compose, JSON-LD, strictly validated Autocrypt headers, staged regular/inline/CID attachments, embedded data-image conversion, MailSo-compatible related/mixed MIME nesting, raw `message/rfc822` handling, and S/MIME/GnuPG root transformation (GnuPG sign/encrypt applies to drafts exactly like sends, sharing PHP's buildMessage order). Saved drafts retain staged capabilities for later retries. Bounded direct client-supplied S/MIME certificate/private-key signing follows the same selected-account/direct/unsigned MailSo precedence as SendMessage; OpenPGP client-side signed/encrypted, selected-account S/MIME, OAuth SMTP, and data-x-src/data-x-style-url transformations are supported. The `filter.build-message` point is native through the opt-in `X-Originating-IP` stamp shared with `SendMessage`. |
...
The active demo-account plugin recipient policy is native. Set
`FRICKMAIL__DEMO_ACCOUNT__EMAIL` to enable it; an empty value disables the
policy. Set the optional `RECIPIENT_DELIMITER` when the deployment permits
subaddressed demo recipients, matching legacy plugin configuration.
...
| Change password | `ChangePassword` (partial-native: Frickmail login password with policy/HIBP/credential-reseal/session rotation is native; external LDAP/arbitrary-SQL/hosting-panel drivers need operator infrastructure and are an operator retire-vs-port decision) |
...
2. Complete `Message` parity: remaining message/header details and detailed
   message payloads. Absent `id`/`preview` now serialize as PHP-exact `""`
   and absent `headers` as an empty `MimeHeaderCollection` object (no longer
   null); flags already match PHP normalization (lowercase, unique,
   `$readreceipt`→`$mdnsent`, `$replied`→`\answered`). Opaque (non-detached) S/MIME `smimeSigned` auto-verification
   is native and best-effort in the `Message` handler (verified inner `body`
   plus `success`, mirroring PHP `DoMessage()`); detached S/MIME (`success`
   only) and PGP (`pgpSigned` replaced with `{fingerprint, success}` via the
   account GnuPG home) auto-verification are now also native when
   `security.auto_verify_signatures` is enabled (default `false`, matching
   PHP), reusing the `PgpVerifyMessage` fetch contract and bounds.
