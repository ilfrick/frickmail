...
| `SendMessage` | `dev/View/Popup/Compose.js` | partial-native | Native compose and asynchronous SMTP delivery for password-backed selected accounts. It preserves the `From` display name, validates bare envelope addresses, de-duplicates recipients, and supports `subject`, `text/plain`, `text/html`, `multipart/alternative`, threading, priority, read-receipt, and legacy `TLS-Required: No` headers. To/Cc/Bcc/Reply-To display names are not yet preserved. SMTP DNS resolution is deadline-bounded, rejects any private/reserved result, and pins one validated public address while retaining the original hostname for TLS verification; the send runs under a bounded semaphore. It deliberately does not retry a whole SMTP transaction on another address after an ambiguous failure, which could duplicate delivery. Transport and Sent copies share one Message-ID and Date; Bcc is omitted in transport but retained in the stored copy. A requested `saveFolder` APPEND must succeed (or return `CantSaveMessage`); fallback occurs only after a definitive rejection, never after a timeout/unknown commit outcome. Reply/forward flagging and draft cleanup remain best-effort. Nonempty unsupported attachment/crypto/linked-data fields are rejected explicitly; SMTP REQUIRETLS, DSN, OAuth SMTP, attachment/crypto MIME assembly, and `identityID` selection remain unmigrated. Compose body/header/recipient bounds prevent the generic 64 MiB request limit from multiplying into excessive memory use. |
| `SaveMessage` | `dev/View/Popup/Compose.js` | partial-native | Natively saves a selected-account draft with `\Seen`, a stable generated `Message-ID`, UID lookup fallback, PHP-compatible `{folder, uid}`/`true` response shapes, and delete-after-append cleanup of the prior draft. It shares the migrated basic compose fields and rejects unsupported attachment/crypto payloads explicitly so draft data is never silently discarded. Full attachment and crypto MIME parity remains coupled to `SendMessage`. |
