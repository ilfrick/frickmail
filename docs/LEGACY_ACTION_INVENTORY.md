...
1. Complete the server-side GnuPG keyring/signing/encryption flow. Client-supplied OpenPGP
   `signed`/`encrypted` MIME content, selected-account and direct S/MIME signing, S/MIME
   certificate encryption, OAuth SMTP, and `data-x-src*`/`data-x-style-url` transformations are
   native. Until the
