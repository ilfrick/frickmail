...
| `MessageList` | `dev/Stores/User/Messagelist.js` | compat-known | Native implementation is staged but dispatch remains disabled until RFC 7377 MULTISEARCH folder scopes with mailbox-correlated results, calendar-relative search filters, permanent search filters, non-default fast-simple-search configuration plumbing, sort/threading, date grouping, RawKey GET server-side UID cache backend interaction, previews, and full cache parity match legacy PHP behavior. `dateTimestamp`/`dateTimestampSource` parsing, including the INTERNALDATE fallback, BODYSTRUCTURE-based attachment presence and metadata, MailSo-compatible encrypted boolean metadata, MailSo-compatible spam metadata, MailSo-compatible read-receipt validation, capability-gated Gmail `X-GM-MSGID` fallback for `id`, MailSo-compatible limit normalization, branch-specific limit defaulting, `uidNext` defaulting, search trimming, injection-safe plain-text/address/subject/body/attachment/state/header/keyword/size/absolute-date search criteria for prefixed and URL-query forms, capability-gated RFC 5032 age intervals, the default four-header fast-simple-search behavior, UTF8=ACCEPT-aware Unicode search with `CHARSET UTF-8` synchronizing literals for classic servers, completion-checked server-side `UID SEARCH`, reported sort and limited metadata normalization, nullable `totalThreads` collection emission, subject trimming, flag alias normalization, `References` whitespace normalization, nullable absent EmailId `id`, nullable absent/empty `preview`, MailSo-compatible 100-entry email collection caps, server-side UID selection with pre-pagination `UNDELETED` filtering, accurate filtered totals, stable newest-UID-first paging, exact-`INBOX` new-message probing, thread-view `newMessages` suppression, the internal legacy collection/message JSON adapter, PHP-compatible optional nested folder-info count/modseq/etag/permanent-flag/append-size fields, the selected-account POST request/fetcher path, the RawKey GET request decoder with `threadAlgorithm` and account-hash payload capture, MailSo-compatible RawKey cache-key/validation-state calculation, and RawKey HTTP cache validator/header emission are staged internally; live dispatch remains pending. |
