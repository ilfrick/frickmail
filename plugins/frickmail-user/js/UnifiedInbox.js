	...
	function loadMessages() {
		...
		r.pluginRemoteRequest((iErr, oData) => {
			...
			if (status) status.textContent = msgs.length + ' messages'
				+ (errors.length ? ' (' + errors.length + ' account' + (errors.length > 1 ? 's' : '') + ' failed)' : '');
			if (!msgs.length && errors.length) {
				if (list) list.innerHTML = '<div style="padding:24px 16px;color:#f38ba8;font-size:.85rem">'
					+ '<strong>Could not reach ' + errors.length + ' account' + (errors.length > 1 ? 's' : '') + ':</strong><br>'
					+ errors.map(e => escHtml(e)).join('<br>') + '</div>';
				return;
			}
